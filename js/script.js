...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      if(isDragging){ //當拖拉為True時
			...
			img.width = canvas.width;//設定canvas的大小需符合profileimg的大小
			img.height = canvas.height;
			ctx.drawImage(img,0,0);//從XY軸0，0值開始畫如profileimg
			ctx.drawImage(img3,150,350);
			...
			ctx.fillText(inputedText, 220,420); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
